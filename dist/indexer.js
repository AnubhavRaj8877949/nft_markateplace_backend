...
async function main() {
    ...
    const marketplaceContract = new ethers_1.ethers.Contract(MARKETPLACE_ADDRESS, MARKETPLACE_ABI, provider);
    const nftContract = new ethers_1.ethers.Contract(NFT_ADDRESS, NFT_ABI, provider);
        ...
        try {
            ...
            if (currentBlock > lastBlock) {
                ...
                for (const log of boughtLogs) {
                    const { buyer, nftAddress, tokenId } = log.args;
                ...
                for (const log of canceledLogs) {
                    const { seller, nftAddress, tokenId } = log.args;
                ...
                for (const log of offerCreatedLogs) {
                    const { offerer, nftAddress, tokenId, price } = log.args;
                ...
                for (const log of offerAcceptedLogs) {
                    const { seller, offerer, nftAddress, tokenId, price } = log.args;
                ...
                for (const log of offerCanceledLogs) {
                    const { offerer, nftAddress, tokenId } = log.args;
...
async function handleTransfer(from, to, tokenId, nftContract) {
    ...
    try {
        ...
        if (!existingNft || normalizedFrom === ethers_1.ethers.ZeroAddress) {
            try {
...
async function handleItemListed(seller, nftAddress, tokenId, price) {
...
async function handleItemBought(buyer, nftAddress, tokenId) {
...
async function handleItemCanceled(seller, nftAddress, tokenId) {
    const normalizedNftAddress = nftAddress.toLowerCase();
...
async function handleOfferCreated(offerer, nftAddress, tokenId, price) {
...
async function handleOfferAccepted(seller, offerer, nftAddress, tokenId, price) {
    const normalizedOfferer = offerer.toLowerCase();
    const normalizedSeller = seller.toLowerCase();
    const normalizedNftAddress = nftAddress.toLowerCase();
...
async function handleOfferCanceled(offerer, nftAddress, tokenId) {
    const normalizedOfferer = offerer.toLowerCase();
    const normalizedNftAddress = nftAddress.toLowerCase();
